...
class AnalogGaugeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.normallmtcolor =0
        self.warnlmtcolor=0
        self.actionlmtcolor=0
    ...
    def update_value(self, value, mouse_controlled = False):
        ...
        if value <= self.value_min:
            ...
            self.value = value
        # self.paintEvent("")
        self.valueChanged.emit(float(value)) # changed from int to float farhan
        # print(self.value)
    ...
    def draw_filled_polygon(self, outline_pen_with, normallmtcolor, warnlimitcolor, actionlmtcolor):
        ...
        if not self.scale_polygon_colors == None:
            ...
            grad.setColorAt(actionlmtcolor, Qt.red)
            grad.setColorAt(warnlimitcolor, Qt.yellow)  # lower the value more is the color on polygon
            grad.setColorAt(normallmtcolor, Qt.green)
    ...
    def create_values_text(self):
        ...
        # angle_distance = (float(self.scale_angle_size) / float(self.scala_main_count))
        # for i in range(self.scala_main_count + 1):
        #text = str(float(self.value))  # changed from int to float by farhan

        textfloat = (float(self.value))  # changed from int to float by farhan
        #textfloat = format(textfloat/10, "10.1E")
        text = str(textfloat)
        ...
        w = fm.width(text) + 1
        h = fm.height()
        painter.setFont(QFont(self.value_fontname, self.value_fontsize-6))
    ...
    def paintEvent(self, event):
        ...
        if self.enable_filled_Polygon:
            ...
            self.draw_filled_polygon(0,self.normallmtcolor,self.warnlmtcolor,self.actionlmtcolor)
...
if __name__ == '__main__':
    ...
    class mainclass():
        ...
        def __init__(self):
            ...
            self.my_gauge.widget.update_value(float(self.my_gauge.widget.value_max - self.my_gauge.widget.value_min)/2)# changed from int to float
